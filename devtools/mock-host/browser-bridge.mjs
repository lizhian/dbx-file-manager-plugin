...
function installBridge(channel) {
  ...
  const pending = new Map(), listeners = { context: new Set(), init: new Set(), event: new Set(), binary: new Set() };
  let resolveReady;
  ...
  const encode = input => {
    ...
    let result = ''; for (let i = 0; i < data.length; i += 8192) result += String.fromCharCode(...data.subarray(i, i + 8192));
    return btoa(result);
  };
  const decode = value => Uint8Array.from(atob(value), c => c.charCodeAt(0));
  const request = (method, params) => new Promise((resolve, reject) => {
    const id = ++sequence;
    const timer = setTimeout(() => { pending.delete(id); reject(new Error('Mock host request timed out')); }, 310000);
    pending.set(id, { resolve, reject, timer });
    parent.postMessage({ source: 'dbx-plugin', version: 1, channel, type: 'request', id, method, params }, '*');
  });
  const listen = (kind, callback) => { listeners[kind].add(callback); return () => listeners[kind].delete(callback); };
  function applyTheme(value) {
    if (!value) return;
    theme = value; document.documentElement.dataset.dbxTheme = value.appearance;
  ...
  window.dbxPlugin = Object.freeze({
    ready, get context() { return context; }, get locale() { return locale; }, get theme() { return theme; }, request,
    ...
    readAssetUrl: async path => { const asset = await request('ui.readAsset', { path }); return URL.createObjectURL(new Blob([decode(asset.dataBase64)], { type: asset.contentType })); },
    ...
    onContext: fn => listen('context', fn), onEvent: fn => listen('event', fn), onBinary: fn => listen('binary', fn),
    onInit: fn => { const off = listen('init', fn); ready.then(() => { if (listeners.init.has(fn)) fn(context); }); return off; },
    encodeBase64: encode, decodeBase64: decode,
  });
  addEventListener('message', event => {
    const m = event.data;
    if (event.source !== parent || m?.source !== 'dbx-host' || m.channel !== channel || m.version !== 1) return;
    if (m.type === 'init') { initialized = true; clearInterval(readyTimer); context = m.context || {}; locale = m.locale || 'zh-CN'; applyTheme(m.theme); resolveReady(); }
    if (m.type === 'context') { context = m.context || {}; for (const fn of listeners.context) fn(context); }
    if (m.type === 'env') { locale = m.locale || locale; applyTheme(m.theme); }
    if (m.type === 'event') for (const fn of listeners.event) fn({ method: m.method, params: m.params });
    if (m.type === 'binary') for (const fn of listeners.binary) fn({ channel: m.binaryChannel, data: decode(m.dataBase64).buffer });
    if (m.type === 'response') {
      const waiter = pending.get(m.id); if (!waiter) return;
      pending.delete(m.id); clearTimeout(waiter.timer);
