...
export function binding(field) { return field.binding || (field.type === 'password' ? 'secret' : 'config'); }
export function validateRecord(manifest, record, required = true) {
  ...
  for (const f of provider.fields) {
    ...
    if (empty) { if (required && f.required) throw new Error(`Missing field: ${f.key}`); continue; }
    ...
    if (binding(f) === 'port' && (!Number.isInteger(value) || value < 1 || value > 65535)) throw new Error('Port must be between 1 and 65535');
...
export function lifecyclePayload(manifest, record) {
  const provider = providerFor(manifest, record.providerId);
  const c = { id: record.id, db_type: 'plugin', plugin_id: manifest.id, plugin_connection_provider: provider.id,
    plugin_connection_type: provider.database_type, name: provider.label, host: '', port: 0, username: '',
    external_config: {}, connection_secrets: {}, read_only: record.readOnly, query_timeout_secs: 60, idle_timeout_secs: 60 };
  for (const f of provider.fields) {
    const value = record.values[f.key] ?? f.default;
...
export class ConnectionStore {
  records = [];
  constructor(directory, manifest) { this.directory = directory; this.manifest = manifest; this.file = join(directory, 'connections.json'); }
  async load() {
  ...
  get(id) { const record = this.records.find(r => r.id === id); if (!record) throw new Error('Connection not found'); return structuredClone(record); }
  async replace(records) {
    ...
    await rename(temporary, this.file); this.records = next;
  }
}
