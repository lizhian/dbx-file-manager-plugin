...
async function copyPath(entry: Entry) { const value = displayPath(entry.uri, root.value); try { await navigator.clipboard.writeText(value); status.value = '路径已复制'; } catch { await ask({ title: '复制路径失败', message: value, confirm: '知道了' }); } }
  ...
  <main class="relative flex h-full min-h-0 flex-col bg-base-100 text-base-content" @click="menu = undefined; downloadsOpen = false" @keydown.esc="menu = undefined; downloadsOpen = false">
    <header class="relative z-20 flex min-h-12 flex-wrap items-center gap-1 border-b border-base-300 px-3 py-2">
      ...
      <form class="flex min-w-0 flex-1 items-center gap-1" @submit.prevent="m.accessPath('directory')">
        ...
        <button type="button" class="btn btn-sm btn-square btn-ghost" title="预览路径中的文件" aria-label="预览路径中的文件" :disabled="busy || !m.can('read')" @click="m.accessPath('preview')"><Eye :size="16" /></button>
        <button type="button" class="btn btn-sm btn-square btn-ghost" title="下载路径中的文件" aria-label="下载路径中的文件" :disabled="busy || !m.can('download')" @click="m.accessPath('download')"><Download :size="16" /></button>
      </form>
    ...
    <div class="flex min-h-0 flex-1">
      <section class="min-h-0 min-w-0 flex-1 overflow-auto" aria-label="文件列表" :aria-busy="busy">
        ...
        <p v-if="!tableRows.length" class="p-8 text-center text-sm text-base-content/60" role="status">{{ busy ? '正在加载…' : capabilities.list === false ? '此连接不支持目录浏览，请输入已知文件路径进行预览或下载。' : '目录为空' }}</p><div v-if="cursor" class="p-3 text-center"><button class="btn btn-sm" :disabled="busy" @click="m.more">加载更多</button></div>
      </section>
      <section v-if="preview" class="absolute inset-0 z-30 flex min-h-0 flex-col bg-base-100 md:static md:z-auto md:w-[42%] md:min-w-72 md:border-l md:border-base-300" aria-label="文件预览">
        ...
        <TextPreview v-if="preview.kind === 'text'" v-model:content="content" :name="preview.entry.name" :editable="m.canEdit.value" :disabled="busy" @keydown.meta.s.prevent="m.save" @keydown.ctrl.s.prevent="m.save" />
