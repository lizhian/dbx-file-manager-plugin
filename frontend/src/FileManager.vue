...
import { ChevronLeft, ChevronRight, ChevronDown, RefreshCw, FolderPlus, Upload, Download, FilePenLine, Trash2, File, FileText, Image, Folder, FolderOpen, Copy, X, Save, CheckCircle2, CircleAlert } from 'lucide-vue-next';
import { formatSize, parentUri, displayPath, type Context, type Entry } from './bridge';
...
const splitArea = ref<HTMLElement>();
const previewWidth = ref(42);
const resizing = ref(false);
function setPreviewWidth(width: number) { previewWidth.value = Math.max(25, Math.min(75, width)); }
function resizePreview(event: PointerEvent) {
  if (!resizing.value || !splitArea.value) return;
  const rect = splitArea.value.getBoundingClientRect();
  if (rect.width) setPreviewWidth((rect.right - event.clientX) / rect.width * 100);
}
...
const copyNotice = ref<{ x: number; y: number; above: boolean }>();
let copyNoticeTimer: ReturnType<typeof setTimeout> | undefined;
let copySequence = 0;
let disposed = false;
async function copyPath(entry: Entry, event: MouseEvent) {
  const button = event.currentTarget as HTMLElement;
  const rect = button.getBoundingClientRect();
  const sequence = ++copySequence;
  clearTimeout(copyNoticeTimer); copyNotice.value = undefined;
  const value = displayPath(entry.uri, root.value);
  try {
    await navigator.clipboard.writeText(value);
    if (disposed || sequence !== copySequence) return;
    const above = rect.top >= 40;
    copyNotice.value = { x: Math.max(64, Math.min(rect.left + rect.width / 2, innerWidth - 64)), y: above ? rect.top - 8 : rect.bottom + 8, above };
    copyNoticeTimer = setTimeout(() => { copyNotice.value = undefined; }, 1800);
  } catch {
    if (!disposed && sequence === copySequence) await ask({ title: '复制路径失败', message: value, confirm: '知道了' });
  }
}
...
onBeforeUnmount(() => { disposed = true; clearTimeout(copyNoticeTimer); cancelClick(); resolve?.(null); });
</script>

<template>
  <main @scroll.capture="copyNotice = undefined" class="relative flex h-full min-h-0 flex-col bg-base-100 text-base-content" @click="menu = undefined; downloadsOpen = false" @keydown.esc="menu = undefined; downloadsOpen = false">
    <header class="relative z-20 flex min-h-12 flex-wrap items-center gap-1 border-b border-base-300 px-3 py-2">
      ...
      <form class="flex min-w-0 flex-1 items-center gap-1" @submit.prevent="m.accessPath(m.can('list') ? 'directory' : 'preview')">
        <input v-model="inputPath" class="input input-sm min-w-0 w-full focus:outline-none focus:ring-0" aria-label="存储路径" :readonly="busy" @keydown.enter.prevent="m.accessPath(m.can('list') ? 'directory' : 'preview')" />
        <button type="button" class="btn btn-sm btn-square btn-ghost" title="进入目录" aria-label="进入目录" :disabled="busy || !m.can('list')" @click="m.accessPath('directory')"><FolderOpen :size="16" /></button>
      </form>
      ...
      <div class="ml-auto flex flex-wrap justify-end gap-1 max-sm:w-full">
        <button class="btn btn-sm btn-outline border-base-300" :disabled="busy || !m.can('mkdir')" @click="startFolderAction('mkdir')"><FolderPlus :size="16" />新建文件夹</button>
        <button class="btn btn-sm btn-outline border-base-300" :disabled="busy || !m.can('upload')" @click="startFolderAction('upload')"><Upload :size="16" />上传</button>
    ...
    <div ref="splitArea" class="flex min-h-0 flex-1" :class="resizing ? 'select-none cursor-col-resize' : ''">
      <section class="min-h-0 min-w-0 flex-1 overflow-auto" aria-label="文件列表" :aria-busy="busy">
        <table class="table table-sm w-full min-w-[420px] table-fixed">
          <thead class="sticky top-0 z-10 bg-base-200"><tr><th>名称</th><th class="hidden w-24 text-right sm:table-cell">大小</th><th class="hidden w-40 lg:table-cell">修改时间</th><th class="w-32 text-right">操作</th></tr></thead>
          <tbody>
            ...
            <tr v-for="row in tableRows" :key="row.entry.uri + (row.more ? ':more' : '')" class="border-b border-base-300 hover:bg-base-200" :data-file-entry-path="row.entry.uri" :aria-selected="selected?.uri === row.entry.uri" tabindex="0" @click="!row.more && entryClick(row.entry, $event)" @dblclick="!row.more && doubleClick(row.entry)" @keydown.enter.prevent="!row.more && doubleClick(row.entry)" @contextmenu.prevent.stop="!row.more && contextMenu(row.entry, $event)">
              ...
              <td class="text-right"><div v-if="!row.more" class="flex justify-end">
                <button v-if="row.entry.kind === 'file'" class="btn btn-xs btn-square btn-ghost" title="下载" aria-label="下载" :disabled="busy || !m.can('download')" @click.stop="action(row.entry, 'download')"><Download :size="16" /></button>
                <button class="btn btn-xs btn-square btn-ghost" title="复制路径" aria-label="复制路径" :disabled="busy" @click.stop="copyPath(row.entry, $event)"><Copy :size="16" /></button>
                <button class="btn btn-xs btn-square btn-ghost" title="编辑" aria-label="编辑" :disabled="busy || !(m.can('rename') || (row.entry.kind === 'file' && m.can('copy')))" @click.stop="startEdit(row.entry)"><FilePenLine :size="16" /></button>
                <button class="btn btn-xs btn-square btn-ghost text-error" title="删除" aria-label="删除" :disabled="busy || !m.can('delete')" @click.stop="action(row.entry, 'delete')"><Trash2 :size="16" /></button>
              </div></td>
            </tr>
          </tbody>
        </table>
        <p v-if="!tableRows.length" class="p-8 text-center text-sm text-base-content/60" role="status">{{ busy ? '正在加载…' : capabilities.list === false ? '此连接不支持目录浏览，请输入已知文件路径并按回车预览。' : '目录为空' }}</p><div v-if="cursor" class="p-3 text-center"><button class="btn btn-sm" :disabled="busy" @click="m.more">加载更多</button></div>
      </section>
      <section v-if="preview" class="file-preview absolute inset-0 z-30 flex min-h-0 flex-col bg-base-100 md:relative md:z-auto md:min-w-72 md:border-l md:border-base-300" aria-label="文件预览" :style="{ '--preview-width': `${previewWidth}%` }">
        <div role="separator" aria-label="调整预览宽度" aria-orientation="vertical" :aria-valuenow="Math.round(previewWidth)" :aria-valuemin="25" :aria-valuemax="75" tabindex="0" class="absolute -left-1 top-0 z-40 hidden h-full w-2 touch-none cursor-col-resize md:block hover:bg-base-300/60 focus-visible:bg-base-300" @pointerdown="startResize" @pointermove="resizePreview" @pointerup="stopResize" @pointercancel="stopResize" @lostpointercapture="stopResize" @keydown.left.prevent="setPreviewWidth(previewWidth + 2)" @keydown.right.prevent="setPreviewWidth(previewWidth - 2)" @keydown.home.prevent="setPreviewWidth(25)" @keydown.end.prevent="setPreviewWidth(75)" />
        <div class="flex h-[37px] min-h-[37px] shrink-0 items-center gap-2 border-b border-base-300 px-3"><h2 class="min-w-0 flex-1 truncate text-sm font-medium" :title="preview.entry.name">{{ preview.entry.name }}<span v-if="dirty" class="ml-1 text-warning">*</span></h2><button v-if="preview.kind === 'text' && !preview.truncated && preview.editable !== false" class="btn btn-sm btn-square btn-ghost" title="保存" aria-label="保存" :disabled="busy || !dirty || !m.canEdit.value" @click="m.save"><Save :size="16" /></button><button class="btn btn-sm btn-square btn-ghost" title="关闭预览" aria-label="关闭预览" :disabled="busy" @click="m.closePreview"><X :size="16" /></button></div>
        <TextPreview :document-id="preview.token" v-if="preview.kind === 'text'" v-model:content="content" :name="preview.entry.name" :editable="m.canEdit.value" :disabled="busy" @keydown.meta.s.prevent="m.save" @keydown.ctrl.s.prevent="m.save" />
    ...
    <div v-if="copyNotice" role="status" class="pointer-events-none fixed z-50 whitespace-nowrap rounded border border-base-300 bg-base-100 px-3 py-2 text-xs shadow-md" :style="{ left: `${copyNotice.x}px`, top: `${copyNotice.y}px`, transform: copyNotice.above ? 'translate(-50%, -100%)' : 'translateX(-50%)' }">路径已复制</div>
    ...
    <div v-if="menu && selected" class="fixed z-40 w-44 rounded border border-base-300 bg-base-100 p-1 shadow-lg" :style="{ left: `${menu.x}px`, top: `${menu.y}px` }" role="menu" @click.stop>
      <button class="btn btn-sm btn-ghost w-full justify-start" role="menuitem" :disabled="busy || !m.can(selected.kind === 'directory' ? 'list' : 'read')" @click="m.open(selected); menu = undefined">{{ selected.kind === 'directory' ? '进入文件夹' : '预览' }}</button>
      <button v-if="selected.kind === 'file'" class="btn btn-sm btn-ghost w-full justify-start" role="menuitem" :disabled="busy || !m.can('download')" @click="action(selected, 'download')">下载</button>
      <button class="btn btn-sm btn-ghost w-full justify-start" role="menuitem" :disabled="busy" @click="copyPath(selected, $event); menu = undefined">复制路径</button>
      <button class="btn btn-sm btn-ghost w-full justify-start" role="menuitem" :disabled="busy || !(m.can('rename') || (selected.kind === 'file' && m.can('copy')))" @click="startEdit(selected)">编辑</button><button class="btn btn-sm btn-ghost w-full justify-start text-error" role="menuitem" :disabled="busy || !m.can('delete')" @click="action(selected, 'delete')">删除</button>
    </div>
    <dialog v-if="editing" open class="modal"><div class="modal-box flex h-auto max-h-[85vh] max-w-2xl flex-col gap-3"><h2 class="flex shrink-0 items-center gap-3 text-base font-semibold"><span>{{ editing.kind === "file" ? "文件夹树" : "重命名" }}</span><span v-if="editing.kind === 'file'" class="min-w-0 truncate text-sm font-normal text-base-content/60" :title="editFolder">{{ editFolder.replace(root, '') || '/' }}</span></h2><FolderPicker v-if="editing.kind === 'file'" :key="editing.uri" v-model="editFolder" :root="root" :browsable="m.can('list')" :load="m.listDirectories" /><input v-model="editName" class="input input-sm w-full shrink-0 focus:outline-none focus:ring-0" aria-label="文件名称" placeholder="文件名称" /><div class="modal-action mt-0 shrink-0"><button class="btn btn-sm" @click="cancelEdit">取消</button><button v-if="editing.kind === 'file'" class="btn btn-sm" :disabled="busy || !m.can('copy') || !editName.trim()" @click="m.copyTo(editFolder, false, editName.trim()); cancelEdit()">复制</button><button v-if="editFolder !== parentUri(editing.uri) && editing.kind === 'file'" class="btn btn-sm" :disabled="busy || !m.can('rename') || !editName.trim()" @click="m.copyTo(editFolder, true, editName.trim()); cancelEdit()">移动</button><button v-if="editFolder === parentUri(editing.uri)" class="btn btn-sm" :disabled="busy || !m.can('rename') || !editName.trim()" @click="m.rename(editName.trim()); cancelEdit()">重命名</button></div></div></dialog>
    ...
    <dialog v-if="folderAction" open class="modal"><div class="modal-box flex h-auto max-h-[85vh] max-w-2xl flex-col gap-3"><h2 class="text-base font-semibold">{{ folderAction === 'mkdir' ? '选择新建目录位置' : '选择上传目录' }}</h2><FolderPicker v-model="editFolder" :root="root" :browsable="m.can('list')" :load="m.listDirectories" /><input v-if="folderAction === 'mkdir'" v-model="editName" class="input input-sm w-full focus:outline-none focus:ring-0" aria-label="文件夹名称" placeholder="文件夹名称" /><div class="modal-action mt-0"><button class="btn btn-sm" @click="cancelFolderAction">取消</button><button class="btn btn-sm" :disabled="folderAction === 'mkdir' && !editName.trim()" @click="finishFolderAction">{{ folderAction === 'mkdir' ? '新建' : '选择文件' }}</button></div></div></dialog>
  </main>
</template>
